...
Note: There is currently no usesLeft and no auto-removal on use. Consumables persist until replaced (effects later may add depletion).
...
upgrades: { permanent: [{ id, count, info }...], slots: [{ id, info }...] }

info includes: { id, name, kind, desc, useCost, acquireCost }
